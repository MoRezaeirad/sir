...
func cmdMCP(projectRoot string, args []string) {
	...
	case "scan":
		cmdMCPScan(projectRoot, args)
	default:
		fatal("usage: sir mcp [status|inventory|wrap|approve|revoke|list|scope|scan] [--agent <id>] [--yes] [--project <path>]\n\nExamples:\n  sir mcp\n  sir mcp wrap --yes\n  sir mcp status --agent gemini\n  sir mcp approve <name>\n  sir mcp approve --all\n  sir mcp revoke <name>\n  sir mcp list\n  sir mcp scope <name> --allow-network --root .\n  sir mcp scan\n  sir mcp scan --quiet   (cron/launchd: silent on success, exit 3 on drift)\n  sir mcp list --project /path/to/repo")
	}
}
