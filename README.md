...
sir is a hook- and tool-boundary layer, not a host firewall. If a tool executor ignores the hook response, sir cannot *prevent* that one call — but it *detects* the violation (a PostToolUse for a tool sir denied means it ran anyway) and locks the session to deny-all so nothing further proceeds; `sir run` adds OS-level prevention. MCP injection detection is regex-based but normalizes away common evasion — zero-width/bidi characters and base64/hex/percent encoding are decoded before matching — so only paraphrase and homoglyph substitution still slip, and those are backstopped by the integrity-flow egress wall, which denies external egress after *any* untrusted content was ingested this turn, whether or not the scanner flagged it. Turn boundaries advance instantly on every user message (`UserPromptSubmit`), and also via a 30-second gap heuristic — so the turn-scoped secret deny floor can reset without any timing race. To keep that reset honest, secret taint is **monotonic**: a session that has *ever* held a secret keeps re-prompting external egress and pushes (an approval prompt, never a silent allow) across turn boundaries until you run `sir unlock`. A secret laundered through model context and re-emitted *verbatim* is still caught: values that enter context (an approved read, an env read, a detected credential) are fingerprinted one-way, and a verbatim re-emission in any outbound payload is denied — only a *paraphrased or transformed* secret slips (see the threat model's context-laundering table). Shell classification is prefix-aware — it decomposes command substitution, backticks, and `eval`, fails closed on `… | sh`, and flags interpreter one-liners that name a sensitive file literally (`python -c "open('.env')"`) as sensitive reads — but it is not a full POSIX parser, so dynamically-constructed or obfuscated paths remain a residual. The default lease intentionally allows push-to-origin, commit, loopback, and delegation — tighten with `sir trust` or a managed policy. If `mister-core` is missing from `PATH`, Go falls back to a deliberately *more restrictive* subset (parity-tested), and a tampered oracle triggers a hard deny on all tool calls. Model-internal reasoning is out of scope. Hook-layer policy (the default `sir install`) is advisory enforcement; OS-level *prevention* exists only under `sir run`, which adds optional containment (network namespace on Linux, `sandbox-exec` on macOS) and is experimental.
